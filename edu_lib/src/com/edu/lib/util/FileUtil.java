...
public class FileUtil {
	...
	public static void zipFiles(String target, String... entries) {
	    ...
	    try {
	    	...
	    	LogUtils.E("FileUtil", "zip file io error", e);
	    } finally {
	    	if (null != out) {
	    		try {
					out.close();
				} catch (IOException e) {
					LogUtils.E("FileUtil", "close zip out stream error", e);
				}
	    	}
	    }
	}
	...
	public static void unzipFromAssets(Context context, String zipFilename, 
        ...
        if (null == zipStream) {
        	LogUtils.E(TAG, "Hanzi audio zip file does not exist - " + zipFilename);
            return;
        }		
	}
    ...
    public static String getFilePureNameWithFixedExt(String filename, String ext) {
    	if (TextUtils.isEmpty(filename)) return null;
    	if (TextUtils.isEmpty(ext)) return getFilePureName(filename);
    ...
    public static void delete(File file) {
        ...
        if (file.exists()) {
            ...
            if (!deleteFlag) {
            	LogUtils.E(TAG, "delete " + file.getAbsolutePath() + " : " + deleteFlag);
            }
        }
    }
